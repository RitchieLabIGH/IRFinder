...
if [ -n "$OPTNONPOLYA" ]; then
	if [[ "$OPTNONPOLYA" == *.gz ]]; then
...
# The omnidirectional exclusion derives from the Blacklist and before from Mappability. 
# It can be empty since now the mappability is given as additional information in the name of the coverage gene.
