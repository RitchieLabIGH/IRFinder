...
void BED12Output::SetOutputStream(std::ostream *os) {
	out = os;
}
...
void BED12Output::ChrMapUpdate(const std::vector<std::string> &chrmap) {
...
void JunctionCount::loadRef(std::istream &IN) {
	...
	while(!IN.eof() && !IN.fail()) {
		...
		if (direction == "-")  {
			chrName_junc_count[s_chr][std::make_pair(start,end)][2] = 1;
		}else if (direction == "+") {
			chrName_junc_count[s_chr][std::make_pair(start,end)][2] = 2;
		}
	}
}
...
int JunctionCount::Directional() const {
	...
	std::cout << "Directionality: Dir evidence:\t" << dir_evidence << "\n";
	std::cout << "Directionality: Nondir evidence:\t" << nondir_evidence << "\n";
	std::cout << "Directionality: Dir evidence known junctions:\t" << dir_evidence_known << "\n";
	std::cout << "Directionality: Nondir evidence known junctions:\t" << nondir_evidence_known << "\n";
	std::cout << "Directionality: Dir matches ref:\t" << dir_same << "\n";
	std::cout << "Directionality: Dir opposed to ref:\t" << dir_diff << "\n";
	std::cout << "Directionality: Dir score all (0-10000):\t" << ((long long)dir_evidence * 10000 / (dir_evidence + nondir_evidence + 1)) << "\n"; //+1 to prevent divide by zero errors.
	long dir_score_known = ((long long)dir_evidence_known * 10000 / (dir_evidence_known + nondir_evidence_known + 1));
	std::cout << "Directionality: Dir score known junctions (0-10000):\t" << dir_score_known << "\n";
...
uint JunctionCount::lookup(std::string ChrName, uint left, uint right, bool direction) const {
	try {
		return chrName_junc_count.at(ChrName).at(std::make_pair(left, right))[direction];
...
uint JunctionCount::lookup(std::string ChrName, uint left, uint right) const {
	try {
		return chrName_junc_count.at(ChrName).at(std::make_pair(left, right))[0] + chrName_junc_count.at(ChrName).at(std::make_pair(left, right))[1];
	}catch (const std::out_of_range& e) {
	}
	return 0;
}
...
void SpansPoint::loadRef(std::istream &IN) {
	...
	for (std::map<std::string, std::vector<uint>>::iterator it_chr=chrName_pos.begin(); it_chr!=chrName_pos.end(); it_chr++) {
...
int FragmentsInROI::WriteOutput(std::ostream *os) const {
	for (std::map<std::string, ulong>::const_iterator itID=RegionID_counter[1].begin(); itID!=RegionID_counter[1].end(); ++itID) {
...
void FragmentsInChr::ChrMapUpdate(const std::vector<std::string> &chrmap) {
