...
class JunctionCount : public ReadBlockProcessor {
	private:
		std::map<std::string, std::map<std::pair<uint,uint>,uint[3]>> chrName_junc_count;
		...
		std::map<std::string, std::map<uint,uint[2]>> chrName_juncLeft_count;
		std::vector<std::map<uint,uint[2]>*> chrID_juncLeft_count;

		std::map<std::string, std::map<uint,uint[2]>> chrName_juncRight_count;
...
class SpansPoint : public ReadBlockProcessor {
	private:
		std::map<std::string, std::vector<uint>> chrName_pos;
		std::map<std::string, std::vector<uint>> chrName_count[2];
...
class FragmentsInChr : public ReadBlockProcessor {
	...
	private:
		std::map<std::string, std::vector<uint>> chrName_count; //only expecting 2 items in our vector.
...
class FragmentsInROI : public ReadBlockProcessor {
	...
	private:
		std::map<std::string, ulong> RegionID_counter[2];
 
		std::map<std::string, std::vector<std::pair<uint,uint>>> chrName_ROI;
		std::map<std::string, std::vector<ulong*>> chrName_count[2];
		...
		std::map<std::string, std::vector<std::string>> chrName_ROI_text;
	public:
		void ProcessBlocks(const FragmentBlocks &blocks);
		void ChrMapUpdate(const std::vector<std::string> &chrmap);
		void loadRef(std::istream &IN);
