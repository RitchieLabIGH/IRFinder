...
class CoverageBlock {
	...
	public:
		uint getLength() { return blockEnd - blockStart;}
		uint getStart(){return blockStart;}
		uint getEnd(){return blockEnd;}
		...
		void updateCoverageArray(std::vector<uint> &arr,std::vector<bool> & covered, uint start, uint end) const;
		void updateCoverageArray(std::vector<uint> &arr,std::vector<bool> & covered, uint start, uint end, bool dir) const;
